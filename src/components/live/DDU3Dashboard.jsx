...
export default function DDU3Dashboard({ data, demo }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[#1c1c1c] bg-black text-white font-digi select-none shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)]">
      {/* Brand bar */}
      ...
      <div className="px-3 pt-3">
        <div className="flex gap-[3px] h-3.5">
          {Array.from({ length: SEGMENTS }).map((_, i) => (
            <div key={i} className="flex-1 rounded-[2px] transition-colors duration-75" style={{ backgroundColor: segColor(i), boxShadow: i < lit ? `0 0 7px ${segColor(i)}` : "none" }} />
          ))}
        </div>
        {shift && (
          <div className="text-center text-[10px] tracking-[0.3em] text-red-500 animate-pulse mt-1">▲ SHIFT ▲</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 items-center px-3 py-2 border-b border-[#1a1a1a]">
        <div className="flex flex-col">
          ...
          <span className="text-4xl font-bold tabular-nums leading-none">{Math.round(data.speed_kmh || 0)}</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-[9px] tracking-[0.2em] text-[#666]">GEAR</span>
          <span className="text-7xl font-bold tabular-nums leading-none" style={{ color: data.gear > 0 ? "#fff" : "#ff9800" }}>
            {data.gear > 0 ? data.gear : "N"}
          </span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-[9px] tracking-[0.2em] text-[#666]">RPM</span>
          <span className="text-4xl font-bold tabular-nums leading-none" style={{ color: shift ? "#ff1744" : "#00e5ff" }}>
            {data.rpm || 0}
          </span>
      ...
      <div className="grid grid-cols-4 gap-px bg-[#1a1a1a] border-b border-[#1a1a1a]">
        <Cell label="LAP" value={data.lap ? `${data.lap}/${data.total_laps || "--"}` : "--"} />
      ...
      <div className="px-3 py-3 border-b border-[#1a1a1a]">
        ...
        <div className="grid grid-cols-2 gap-2 max-w-[280px] mx-auto">
          <TyreCell label="FL" t={data.tyres?.fl} />
          <TyreCell label="FR" t={data.tyres?.fr} />
          <TyreCell label="RL" t={data.tyres?.rl} />
          <TyreCell label="RR" t={data.tyres?.rr} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-px bg-[#1a1a1a]">
        ...
        <div className="bg-black p-3">
          ...
          <div className="text-2xl font-bold tabular-nums mt-1">{data.fuel_litres != null ? data.fuel_litres.toFixed(1) : "--"}<span className="text-sm text-[#666]"> L</span></div>
...
function TyreCell({ label, t }) {
  ...
  return (
    <div className="rounded border border-[#222] p-2" style={{ background: `linear-gradient(135deg, ${tempColor(temp)}22, transparent)` }}>
      ...
      <div className="text-xl font-bold tabular-nums leading-none my-0.5" style={{ color: tempColor(temp) }}>{temp != null ? Math.round(temp) : "--"}°</div>
