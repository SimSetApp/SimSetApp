...
export default function DDU3Dashboard({ data, demo }) {
  ...
  const { config, update, toggle, reset } = useDashboardConfig();
  ...
  const gearRem = (fs ? 8 : 3.75) * config.gearScale;

  const Handle = () => (
    <PanelResizeHandle className="w-1.5 bg-[#161616] hover:bg-primary/50 transition-colors flex items-center justify-center group">
      <div className="w-0.5 h-8 rounded-full bg-[#333] group-hover:bg-primary/70" />
    </PanelResizeHandle>
  );
  ...
  return (
    <div className="space-y-3">
      {customize && !fs && (
        <DashboardCustomizer config={config} update={update} toggle={toggle} reset={reset} onClose={() => setCustomize(false)} />
      )}
      <div
        ref={ref}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between px-1.5 py-1 text-[10px] border-b" style={{ borderColor: C.border }}>
              ...
              <div className="flex items-center gap-3">
                <span><span style={{ color: C.label }}>RPM </span><span className="tabular-nums font-bold" style={{ color: shift ? C.red : accent }}>{data.rpm || 0}</span></span>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setCustomize((c) => !c)} className="p-0.5 rounded text-[#777] hover:text-white hover:bg-white/10 transition-colors" aria-label="Customise dashboard">
            ...
            {/* Body: resizable columns */}
            <PanelGroup
              direction="horizontal"
              className="px-1.5 py-1.5"
              onLayout={(l) => update({ layout: l })}
            >
              <Panel defaultSize={config.layout[0]} minSize={15}>
              ...
              <Handle />
              <Panel defaultSize={config.layout[1]} minSize={15}>
                <div className="h-full space-y-1 overflow-hidden">
                  {show.fuel && (
                  ...
                  {show.gear && (
                    <div className="flex flex-col items-center py-0.5">
                      <span className="font-bold tabular-nums leading-[0.8]" style={{ fontSize: `${gearRem}rem`, color: data.gear > 0 ? C.text : C.amber, textShadow: shift ? `0 0 30px ${C.red}` : "none" }}>
              ...
              <Handle />
              <Panel defaultSize={config.layout[2]} minSize={15}>
                <div className="h-full space-y-1 overflow-hidden">
                  {show.delta && (
                    <div className="rounded border p-1.5 space-y-0.5 text-[10px]" style={{ borderColor: C.border, background: C.panel }}>
                      <Row label="LAST LAP" value={fmt(data.last_lap_time)} />
                  ...
                  <div className="rounded border p-1.5 space-y-0.5 text-[10px]" style={{ borderColor: C.border, background: C.panel }}>
                    ...
                    <Row label="CURRENT" value={fmt(data.current_lap_time)} valueColor={accent} />
                  </div>
                  {show.cars && (
                    <div className="rounded border p-1.5 space-y-0.5 text-[10px]" style={{ borderColor: C.border, background: C.panel }}>
            ...
            {show.bottom && (
              <div className="flex gap-1 px-1.5 pb-1.5">
                ...
                <MiniBox label="TC1" value="0" border={accent} />
