...
function tempGradient(t,T){if(t==null)return T.label;const S=[[30,217,91,60],[70,180,85,52],[86,150,100,50],[96,54,100,52],[108,0,100,55],[130,0,100,42]];if(t<=S[0][0])return'hsl('+S[0][1]+','+S[0][2]+'%,'+S[0][3]+'%)';if(t>=S[5][0])return'hsl('+S[5][1]+','+S[5][2]+'%,'+S[5][3]+'%)';for(let i=0;i<5;i++){if(t>=S[i][0]&&t<S[i+1][0]){const f=(t-S[i][0])/(S[i+1][0]-S[i][0]);return'hsl('+(S[i][1]+(S[i+1][1]-S[i][1])*f)+','+(S[i][2]+(S[i+1][2]-S[i][2])*f)+'%,'+(S[i][3]+(S[i+1][3]-S[i][3])*f)+'%)';}}return'hsl(0,100%,50%)';}
function bloom(c,i){return i?'0 0 8px '+c+',0 0 20px '+c+'88,0 0 40px '+c+'44':'0 0 6px '+c+'bb,0 0 16px '+c+'55';}
...
function wRpmGear(d,w,h,T,u){
  ...
  let s='';for(let i=0;i<segs;i++){const f=i/segs,lit=rpmPct>=f+1/segs*0.5,col=f<0.6?T.ledGreen:f<0.85?T.ledYellow:T.ledRed,on=lit&&(!shift||flash);s+='<div style="flex:1;border-radius:2px;'+(on?'background:radial-gradient(circle,'+col+' 0%,'+col+' 60%,'+col+'88 100%);box-shadow:0 0 10px '+col+',0 0 4px '+col+',inset 0 0 3px rgba(255,255,255,0.6);opacity:1':'background:rgba(255,255,255,0.04);box-shadow:inset 0 0 3px rgba(0,0,0,0.5),inset 0 1px 1px rgba(0,0,0,0.3);opacity:0.35')+'"></div>';}
...
function wRpmBar(d,w,h,T){
  ...
  for(let i=0;i<segs;i++){const f=i/segs,lit=rpmPct>=f+1/segs*0.5,col=f<0.6?T.ledGreen:f<0.85?T.ledYellow:T.ledRed,on=lit&&(!shift||flash);s+='<div style="flex:1;border-radius:9999px;height:78%;'+(on?'background:radial-gradient(circle,'+col+' 0%,'+col+' 60%,'+col+'88 100%);box-shadow:0 0 10px '+col+',0 0 4px '+col+',inset 0 0 3px rgba(255,255,255,0.6);opacity:1':'background:rgba(255,255,255,0.04);box-shadow:inset 0 0 3px rgba(0,0,0,0.5),inset 0 1px 1px rgba(0,0,0,0.3);opacity:0.35')+'"></div>';}
...
function wCars(d,w,h,T){const g=v=>v==null?'--.---':(v>0?'+':'')+v.toFixed(3),fs=Math.max(14,Math.min(h*0.22,w*0.11));return '<div style="width:100%;height:100%;padding:8px;display:flex;flex-direction:column;gap:8px;justify-content:center"><div>'+title('CAR AHEAD',T)+'<div class="fd" style="font-weight:700;font-size:'+fs+'px;color:'+T.ledGreen+';text-shadow:'+bloom(T.ledGreen)+';line-height:1">'+g(d.car_ahead_gap)+'</div></div><div>'+title('CAR BEHIND',T)+'<div class="fd" style="font-weight:700;font-size:'+fs+'px;color:'+T.ledRed+';text-shadow:'+bloom(T.ledRed)+';line-height:1">'+g(d.car_behind_gap)+'</div></div></div>';}
